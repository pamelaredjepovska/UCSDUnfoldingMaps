...
public class EarthquakeCityMap extends PApplet {
	...
	private void sortAndPrint(int numToPrint) {
		// An array from the list of earthquake markers
		List<EarthquakeMarker> quakeList = new ArrayList<EarthquakeMarker>();
		for (Marker marker : quakeMarkers) {
			quakeList.add((EarthquakeMarker) marker);
		}

		// Sort the list of earthquake markers in reverse order based on magnitude
		Collections.sort(quakeList);

		// If numToPrint > number of earthquakes, print all the earthquakes
		if (numToPrint > quakeList.size()) {
			numToPrint = quakeList.size();
		}

		// Print the top numToPrint earthquakes
